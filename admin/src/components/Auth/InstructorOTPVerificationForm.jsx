...
const InstructorOTPVerificationForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:8081/api/instructors/verify-otp', { 
        otp: otpCode,
        email: email 
      });
      
      // Store the reset token for the next step
      sessionStorage.setItem('instructorResetToken', response.data.resetToken);
      ...
      setTimeout(() => {
        navigate('/instructor/reset-password/new-password');
      }, 1500);
  ...
  const handleResendOTP = async (e) => {
    ...
    try {
      await axios.post('http://localhost:8081/api/instructors/send-otp', { email });
...
export default InstructorOTPVerificationForm;