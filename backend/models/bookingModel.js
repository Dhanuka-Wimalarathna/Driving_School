...
// Check if student already has a booking for a specific vehicle category on a specific date
export const checkStudentVehicleCategoryBooking = (student_id, date, vehicle, callback) => {
  const sql = `
    SELECT COUNT(*) AS count 
    FROM bookings 
    WHERE student_id = ? 
  ...
  sqldb.query(sql, [student_id, date, vehicle], (err, results) => {
    if (err) return callback(err);

    const hasBooking = results[0].count > 0;
    callback(null, hasBooking);
  });
};
